...
const sections: Section[] = [
  ...
  {
    id: 'loyalty',
    title: '10. Бонусы и подарочные сертификаты',
    icon: Sparkles,
    description: 'Программа лояльности — копите баллы и приводите друзей',
    steps: [
      { text: 'Откройте раздел «Мой кабинет» → вкладка «Бонусы».' },
      { text: 'В блоке «Текущий баланс» видите количество накопленных баллов (1 балл = 1 ₸).' },
      { text: 'Баллы автоматически начисляются после каждой оплаты счёта в клинике (процент устанавливает клиника).' },
      { text: 'Чтобы списать баллы — сообщите администратору при оплате счёта. Можно списать до фиксированного % от суммы (обычно 30%).' },
      { text: 'В «Истории» отображаются все начисления (зелёные +) и списания (красные −).' },
      { text: 'Реферальный код: ваш уникальный код виден на вкладке «Бонусы» (кнопка «Скопировать»). Поделитесь им с другом — когда он будет регистрироваться в клинике, он должен назвать ваш код администратору (или ввести в поле «Реферальный код» при оформлении). После сохранения вы оба автоматически получите бонусные баллы.' },
      { text: 'Если у вас есть подарочный сертификат — назовите его код администратору при оплате.' },
    ],
    tips: [
      'Баллы не сгорают, если в настройках клиники не указан срок действия.',
      'Сертификат можно использовать только один раз и только полностью.',
    ],
  },
];
...
const fullJourney = [
  ...
  'В разделе «Бонусы» накапливаете баллы за каждую оплату и можете их списать при следующих визитах.',
